...
export async function calculateScore(address: string, prisma: PrismaClient, unstoppableName?: string | string[]) {
  ...
  let ACHIEVEMENTS = getAchievements(1);
    ...
    if (!error) {
      ...
      if (!cached || process.env.DEVELOPMENT) {
        ...
        if (unstoppableName && typeof unstoppableName === 'string') {
          ...
          const ensName = await reverseLookup(address.toLowerCase(), web3);
        ...
        const urls = [
          ...
          // Season 2
          // `https://api.polygonscan.com/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&sort=asc&apikey=12C35YEDHAVHNVR1D8NSJKQPK34KKC7EY1`,
          // `https://api.polygonscan.com/api?module=account&action=tokennfttx&address=${address}&startblock=0&endblock=99999999&sort=asc&apikey=12C35YEDHAVHNVR1D8NSJKQPK34KKC7EY1`,
          // `https://api.polygonscan.com/api?module=account&action=tokentx&address=${address}&startblock=0&endblock=99999999&sort=asc&apikey=12C35YEDHAVHNVR1D8NSJKQPK34KKC7EY1`,
        ...
        try {
          ...
          const tokens = results[1] && results[1].tokens || false;
          ...
          if (!transactions || !tokens || !erc721Transactions || !erc20Transactions || !poaps) {
            throw new Error('Error fetching upstream data')
          }

          // Polygon
          // const polygonTransactions = results[6] && typeof results[6].result === "object" && results[6].result || [];
          // const polygonErc721Transactions = results[7] && typeof results[7].result === "object" && results[7].result || [];
          // const polygonErc20Transactions = results[8] && typeof results[8].result === "object" && results[8].result || [];
          ...
          throw new Error('Error fetching data from upstream provider');
        }
